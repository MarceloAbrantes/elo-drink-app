...
jest.mock('@/app/api/contatos/contact/route', () => ({
  GET: mockGetHandler,
  POST: mockPostHandler,
}));

// Import the actual route file before mocking it
const actualRoute = require('@/app/api/contatos/contact/route');

describe('Contact API Route', () => {
  let mockRequest: NextRequest;

  beforeEach(() => {
    mockRequest = new NextRequest('http://localhost:3000/api/contatos/contact');
    // Clear all mocks before each test
    jest.clearAllMocks();
  });

  it('should handle GET requests', async () => {
    const { GET } = require('@/app/api/contatos/contact/route');
    await GET(mockRequest);
    expect(mockGetHandler).toHaveBeenCalledWith(mockRequest);
  });

  it('should handle POST requests', async () => {
    const { POST } = require('@/app/api/contatos/contact/route');
  ...
  it('should handle authentication with valid credentials', async () => {
    const { POST } = require('@/app/api/contatos/contact/route');
    const request = new NextRequest('http://localhost:3000/api/contatos/contact', {
      method: 'POST',
      body: JSON.stringify({
        name: 'Test Contact',
